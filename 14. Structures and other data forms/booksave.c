...
int main (void)
{
	...
	{
		fputs("Не удается открыть файл books.dat\n", stderr);
		exit(1);
	}
	...
	{
		if (count == 0)
			puts("Текущее содержимое файла books.dat:");
	...
	if (count == MAXBKS)
	{
		fputs("Файл books.dat полон.", stderr);
		exit(2);
	}
	...
	if (fclose(pbooks))
		fputs("Ошибка при закрытии файла books.dat\n", stderr);
